...
positivi_dati_precedenti = readtable("iss_bydate_italia_positivi.csv", 'Range', "A218:C398");
...
yStimato = circshift(U,D(1)+1);
yStimato(1:D(1)+1) = U_prev(l-D(1)+1:l+1);
corr = corrcoef(yStimato,Y);
max_corr = corr(1,2);
max_corr_1 = corr(1,2);
DStimato = D(1);
for i = 1:30
	yStimato = circshift(U,D(i)+1);
...
yStimato = circshift(U,DStimato+1);
yStimato(1:DStimato+1) = U_prev(l-DStimato+1:l+1);
muStimato = yStimato\Y;
